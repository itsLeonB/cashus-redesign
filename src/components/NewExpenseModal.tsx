...
import { useFriendships, useCreateGroupExpense, useCalculationMethods } from "@/hooks/useApi";
...
export function NewExpenseModal({ open, onOpenChange }: NewExpenseModalProps) {
  ...
  const [items, setItems] = useState<ExpenseItem[]>([{ name: "", amount: "", quantity: 1 }]);
  ...
  const handleFeeChange = (index: number, field: keyof OtherFee, value: string) => {
    const newFees = [...fees];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const expense = await createExpense.mutateAsync({
        ...
        otherFees: fees.filter(fee => fee.name && fee.amount).map(fee => ({
          name: fee.name,
          amount: fee.amount,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="ghost" size="sm" onClick={handleAddFee}>
                <Plus className="h-4 w-4 mr-1" />
                Add Fee
              </Button>
            </div>
                ...
                {fees.map((fee, index) => (
                  <div key={index} className="flex items-center gap-2">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => handleFeeChange(index, "calculationMethod", value)}
                    >
          ...
          <div className="rounded-lg bg-muted/30 p-4 space-y-2">
            ...
            {feesTotal > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <span className="tabular-nums">${feesTotal.toFixed(2)}</span>
              </div>
            )}
