...
import { AuthResponse, LoginRequest, RegisterRequest, ResetPasswordRequest, UserProfile } from './types';
...
export const authApi = {
  ...
  verifyRegistration: (token: string) => 
    apiClient.get<{ message: string }>(`/auth/verify-registration?token=${token}`),
  ...
  getOAuthUrl: (provider: string) => 
    `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000/api'}/auth/${provider}`,
};
